...
export default function MyProjects() {
  ...
  const sendUpdate = async (event) => {
    ...
    try {
      const response = await axios.put(`${URL}/projects/${id}`, { createdByUserId, title, description });
  ...
  const createProject = async (event) => {
    ...
    try {
      const response = await axios.post(`${URL}/projects`, { createdByUserId, title, description });
